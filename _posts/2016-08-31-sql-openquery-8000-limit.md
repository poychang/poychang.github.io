...
上面這是 [EXECUTE MSDN](https://msdn.microsoft.com/zh-tw/library/ms188332(v=sql.120).aspx) 的 Scheme，以簡單的實例來說，就是用像這樣的語句：`EXECUTE (@Query) AT LinkedServer`，其中 `@Query` 就可以使用 NVARCHAR(MAX) 的變數型態，讓 8000 的字元限制消失。

>NVARCHAR(MAX) 可存放多達 2GB 的資料量，目的是為了取代舊 SQL Server 的 `text`、`ntext`、`image` 等型態

此文件中也說明 `@string_variable` 這是區域變數的名稱。可以是任何 `char`、`varchar`、`nchar` 或 `nvarchar` 資料類型，其中包含 (max) 資料類型在內。
